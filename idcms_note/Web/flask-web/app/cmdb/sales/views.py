...
from app import db
from app.models import Sales, Rack, IpPool, Cabinet
...
thead = [
    [0, u'销售','sales'], [1,u'联系方式', 'contact'], [2,u'邮箱地址', 'email'],
    [3, u'备注' ,'remark']
]
# url分页地址函数
endpoint = '.sales'
...
def sales():
    ...
    if request.method == "POST" and \
        ...
        if sales_form.validate_on_submit():
            sales = Sales(
                ...
                email=sales_form.email.data,
                remark=sales_form.remark.data
            )
            db.session.add(sales)
            db.session.commit()
            value = (
                "usrename:%s isp:%s contact:%s email:%s","remark:%s"
            ) % (sales.username, sales.contact, sales.email, sales.remark)
            record_sql(current_user.username, u"创建", u"销售",
                       sales.id, "Sales", value)
            flash(u'销售添加成功')
        else:
    ...
    if request.method == "GET":
        ...
        if search:
            ...
            sidebarclass = init__sidebar('edititem')
            res = search_res(sales, 'sales' , search)
            if res:
...
def sales_delete():
    ...
    if sales:
        if Rack.query.filter_by(sales=sales.sales).first():
            return u"删除失败 这个销售有机架在使用"
        if IpPool.query.filter_by(sales=sales.sales).first():
            return u"删除失败 这个销售有IP子网在使用"
        if Cabinet.query.filter_by(sales=sales.sales).first():
            return u"删除失败 这个销售有设备在使用"
        record_sql(current_user.username, u"删除", u"销售",
                   sales.id, "sales", sales.sales)
...
def sales_change():
    ...
    if sales:
        ...
        if res == "OK":
            record_sql(current_user.username, u"更改", u"销售",
                        sales.id, item, value)
            setattr(sales, item, value) 
            db.session.add(sales)
            return "OK"
        return res 
    return u"更改失败没有找到该销售"
